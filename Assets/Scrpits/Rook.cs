...
public class Rook : MonoBehaviour
{
    ...
    public Move[,] CheckPossibleMoves()
    {
        ...
        while(index < 8)
        {
            Vector3 pos = new Vector3(transform.position.x + (1f * index), transform.position.y, transform.position.z);
            if (canMove)
            {
    ...
    public bool[] isMoveAllowed(Vector3 nowPos, Vector3 wantedPos, float distance, bool highlightOnce)
    {
        ...
        if (nowPos.x + distance == wantedPos.x && nowPos.z == wantedPos.z && tagOfObject != gameObject.tag && highlightOnce)
        {
        ...
        else if(nowPos.x - distance == wantedPos.x && nowPos.z == wantedPos.z && tagOfObject != gameObject.tag && highlightOnce)
        {
            bool[] returnArray = { true, true };
            if (tagOfObject.Equals("Black") && gameObject.CompareTag("White"))
            {
    ...
    public string DetermineTheTag(Vector3 wantedPos)
    {
        ...
        if (Physics.Raycast(ray, out hit, 1f))
        {
            ...
            {
                if(gameObject.tag.Equals("Black") && hit.collider.gameObject.name.Equals("king white"))
                {
                    chess.canEatKing = true;
                }
                else if(gameObject.tag.Equals("White") && hit.collider.gameObject.name.Equals("king black"))
                {
                    chess.canEatKing = true;
                }
