...
public class King : MonoBehaviour
{
    ...
    {
       // Including it's position there are eight position that can king move.
        ...
        bool canMove = isMoveAllowed(pos, new Vector3(pos.x + 1f, pos.y, pos.z));
        moveArray[1] = new Move(new Vector3(pos.x + 1f, pos.y, pos.z), canMove);

        // two up one left

        canMove = isMoveAllowed(pos, new Vector3(pos.x - 1f, pos.y, pos.z));
        moveArray[2] = new Move(new Vector3(pos.x - 1f, pos.y, pos.z), canMove);

        // two right one up

        canMove = isMoveAllowed(pos, new Vector3(pos.x, pos.y, pos.z + 1f));
        moveArray[3] = new Move(new Vector3(pos.x, pos.y, pos.z + 1f), canMove);

        // two up one right

        canMove = isMoveAllowed(pos, new Vector3(pos.x, pos.y, pos.z - 1f));
        moveArray[4] = new Move(new Vector3(pos.x, pos.y, pos.z - 1f), canMove);

        // two down one left

        canMove = isMoveAllowed(pos, new Vector3(pos.x + 1f, pos.y, pos.z + 1f));
        moveArray[5] = new Move(new Vector3(pos.x + 1f, pos.y, pos.z + 1f), canMove);

        // two down one right

        canMove = isMoveAllowed(pos, new Vector3(pos.x - 1f, pos.y, pos.z + 1f));
        moveArray[6] = new Move(new Vector3(pos.x - 1f, pos.y, pos.z + 1f), canMove);

        // two down one left

        canMove = isMoveAllowed(pos, new Vector3(pos.x + 1f, pos.y, pos.z - 1f));
        moveArray[7] = new Move(new Vector3(pos.x + 1f, pos.y, pos.z - 1f), canMove);

        // two down one right

        canMove = isMoveAllowed(pos, new Vector3(pos.x - 1f, pos.y, pos.z - 1f));
        moveArray[8] = new Move(new Vector3(pos.x - 1f, pos.y, pos.z - 1f), canMove);
    ...
    public bool isMoveAllowed(Vector3 nowPos, Vector3 wantedPos)
    {
        ...
        if (nowPos.x + 1f == wantedPos.x && nowPos.z == wantedPos.z && tagOfObject != gameObject.tag)
        {
            return true;
        }
        else if (nowPos.x - 1f == wantedPos.x && nowPos.z == wantedPos.z && tagOfObject != gameObject.tag)
        {
            return true;
        }
        else if (nowPos.x == wantedPos.x && nowPos.z + 1f == wantedPos.z && tagOfObject != gameObject.tag)
        {
            return true;
        }
        else if (nowPos.x == wantedPos.x && nowPos.z - 1f == wantedPos.z && tagOfObject != gameObject.tag)
        ...
        else if (nowPos.x + 1f == wantedPos.x && nowPos.z + 1f == wantedPos.z && tagOfObject != gameObject.tag)
        {
            return true;
        }
        else if (nowPos.x - 1f == wantedPos.x && nowPos.z + 1f == wantedPos.z && tagOfObject != gameObject.tag)
