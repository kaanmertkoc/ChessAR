...
public class Bishop : MonoBehaviour
{
    ...
    //This method takes the coordinates of where the player clicked and if it is available, moves the object to that coordinate.
    ...
    //This method checks whether there is a space to move around the game object or not with the help of the Move array. If there is no space, it returns false.
    public bool CheckIfCanItMove(Move[,] array)
    {
        int counter = 0;
        // I didn't understand the syntax for checking array's first and second dimension's length. Since i know it must be 4 to 8 i use this way.
        for (int i = 0; i < 4; i++)
        {
            for(int j = 0; j < 8; j++)
            {
    ...
    // This method checks if wanted position is in the move array and if it is available.
    ...
    //This method provides the "eating" process in another words when the initial game object crashes into another game object, it destroys the crashed one. Also if the destroyed object is king, determines who won the game. 
    ...
    //This method checks if the selected move of the player matches the possible moves of the initial game object.
    public Move[,] CheckPossibleMoves()
    {
        // This is c# syntax for declaring a 2d array length of 4 and 8. I know it is dumb.
        Move[,] moveArray = new Move[4, 8];

        // Setting the objects current position to false.
        moveArray[0, 0] = new Move(transform.position, false);
        moveArray[1, 0] = new Move(transform.position, false);
        moveArray[2, 0] = new Move(transform.position, false);
        moveArray[3, 0] = new Move(transform.position, false);
    ...
    // This method creates plane if coordinates are in move array.
    ...
    // Destroys the objects with given tag.
